...
if __name__ == '__main__':
    ...
    model = rnn.RNN('LSTM', config.get_initializer('random_uniform', minval=-0.1, maxval=0.1), "models/LSTM")
    model.add_cell(rnn.BasicLSTMCell, num_units=200)
    model.add_cell(rnn.BasicLSTMCell, num_units=200)
    model.set_input([None], tf.int32, 10000, 200)
    model.set_output([None, 10000], tf.float32)
    model.set_target([None], tf.int32)
    model.set_loss_func(config.get_loss_func("sequence_loss"))
    model.compile()
    model.add_trainer(batch_size, train_steps, keep_prob, trainer.get_optimizer("GradientDescent"), test_lr_decay,
                      trainer.get_gradient_clipper("global_norm", clip_norm=5.0))
