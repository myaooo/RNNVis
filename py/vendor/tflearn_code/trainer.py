        ...
        if scope_for_restore is not None:  # allow variables to be restored into a different scope
            ...
            def vn_map_func(existing_name):  # variable name map function which removes the scope name, e.g.
                if not existing_name.startswith(
                        sname):  # so that "scope_name/var_name/... is retrieved from var_name/...
...
class TrainOp(object):
    ...
    def initialize_training_ops(self, i, session, tensorboard_verbose,
        ...
        self.validation_monitors_T = [tf.Variable(0., name='%s_T' % v.name.rsplit(':', 1)[0], trainable=False) for v in
        ...
        self.val_monitors_P = [tf.placeholder(dtype=tf.float32, name='placeholder/%s' % v.name.rsplit(':')[0]) for v in
                               self.validation_monitors_T]
    ...
    def _train(self, training_step, snapshot_epoch, snapshot_step,
        ...
        if snapshot and self.val_feed_dict:
            ...
            eval_ops = [
                           self.loss] + self.validation_monitors  # compute loss as well as any extra validation monotor tensors
            if show_metric and self.metric is not None:
